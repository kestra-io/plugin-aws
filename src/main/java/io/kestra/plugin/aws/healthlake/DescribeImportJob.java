...
@NoArgsConstructor
@Schema(
    title = "Describe an Amazon HealthLake FHIR import job",
    description = """
        Returns the current status, timing, and error details of a HealthLake bulk import job.
...
@Plugin(
    examples = {
        @Example(
            title = "Poll the status of a HealthLake import job.",
            full = true,
            code = """
...
public class DescribeImportJob extends AbstractConnection implements RunnableTask<DescribeImportJob.Output> {

    @Schema(title = "Data store ID", description = "The ID of the FHIR data store the import job belongs to.")
    @PluginProperty(group = "main")
    @NotNull
    private Property<String> datastoreId;

    @Schema(title = "Job ID", description = "The ID of the import job to describe.")
    @PluginProperty(group = "main")
    @NotNull
